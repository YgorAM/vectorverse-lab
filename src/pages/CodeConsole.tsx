...
export default function CodeConsole() {
  ...
  const runCode = useCallback(() => {
    ...
    if (language === 'javascript') {
      ...
      // Simulate execution for Python/Java
      const matchingExample = codeExamples.find(ex => {
        const exCode = ex.code[language];
        // Check if the current code closely matches an example
        return code.trim() === exCode.trim();
      });

      if (matchingExample) {
        setTimeout(() => setOutput(matchingExample.output[language]), 300);
      } else {
        setTimeout(() => setOutput(`[Simulated ${language} output]\n\n⚠️ Only preloaded examples can be simulated.\nJavaScript code runs directly in the browser.\n\nTip: Load an example from the sidebar or Examples page.`), 300);
      }
    }
  ...
  return (
    <div>
      <p className="font-mono text-xs text-primary mb-2">// console</p>
      <h1 className="text-3xl font-bold mb-2">Code Console</h1>
      <p className="text-muted-foreground mb-6">Write and run vector & matrix code.</p>

      <div className="grid lg:grid-cols-[200px_1fr] gap-4">
        {/* Sidebar */}
        <div className="bg-card border border-border rounded-lg p-3">
          <p className="font-mono text-xs text-muted-foreground mb-2">Examples</p>
